...
- Dashboard: Streamlit + Plotly
    ...
    with tab_c:
        st.markdown("**Struktur File Proyek**")
        st.code("""anxiety_dashboard/
├── app.py                   ← file utama dashboard
├── requirements.txt         ← dependencies
├── best_model.pkl           ← model terbaik (dari notebook)
├── scaler.pkl               ← StandardScaler
└── feature_columns.pkl      ← daftar nama fitur""", language="text")

        st.markdown("**Instalasi & Menjalankan Lokal**")
        st.code("""# 1. Install dependencies
pip install -r requirements.txt

# 2. Salin file .pkl dari output Google Colab
#    best_model.pkl · scaler.pkl · feature_columns.pkl

# 3. Jalankan dashboard
streamlit run app.py""", language="bash")

        st.markdown("**Deploy ke Streamlit Cloud**")
        st.code("""# 1. Push folder ke repository GitHub
# 2. Buka share.streamlit.io
# 3. Hubungkan repo → pilih app.py → Deploy""", language="bash")

        st.markdown("""<div class="info-box">
            💡 Tambahkan <code>.streamlit/config.toml</code> agar tampilan konsisten:<br><br>
            <code>[theme]<br>base = "dark"<br>primaryColor = "#38bdf8"</code>
        </div>""", unsafe_allow_html=True)

        st.markdown("**Dependencies Utama**")
        deps = [
            ("streamlit","≥ 1.32.0","Framework dashboard"),
            ("plotly","≥ 5.20.0","Visualisasi interaktif"),
        ...
        st.dataframe(pd.DataFrame(deps,columns=["Library","Versi","Fungsi"]),
